...
typedef struct nvm_lun_mgmt *(nvmm_reserve_lun)(struct nvm_dev *, int,
						struct gendisk *);
typedef void (nvmm_release_lun)(struct nvm_dev *, int, struct nvm_lun_mgmt *);
...
static inline void print_ppa(struct ppa_addr *p, char *msg, int error)
{
	if (p->c.is_cached) {
		...
		pr_err("ppa: (%s: %x) %llx: ch:%d,pl:%d,lun:%d,blk:%d,pg:%d,sec:%d\n",
			msg, error, p->ppa,
			p->g.ch, p->g.pl, p->g.lun,
			p->g.blk, p->g.pg, p->g.sec);
	}
}
