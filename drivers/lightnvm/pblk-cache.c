...
static int __pblk_write_gc_to_cache(struct pblk *pblk, void *data,
				    u64 *lba_list,
...
int pblk_write_gc_to_cache(struct pblk *pblk, void *data, u64 *lba_list,
			   struct pblk_kref_buf *ref_buf, unsigned int nr_secs,
			   unsigned int nr_rec_secs, unsigned long flags,
			   struct pblk_block *gc_rblk)
{
	int ret;
