...
{
	return sprintf(buf, "%u\n", atomic_read(&pblk->inflight_user));
}
...
static ssize_t pblk_sysfs_gc_blks(struct pblk *pblk, char *buf)
{
	struct pblk_lun *rlun;
	struct nvm_lun *lun;
	struct pblk_block *rblk;
					...
					ssize_t len)
{
	...
	for (i = lba_init; i < lba_end; i++) {
		...
		if (ppa_empty(ppa)) {
			pr_err("pblk: lba:%lu - ppa: EMPTY ADDRESS\n", i);
		} else {
			if (ppa.c.is_cached) {
				pr_err("pblk: lba:%lu - ppa: cacheline:%llu\n",
					i,
					(u64)ppa.c.line);
			...
			pr_err("pblk: lba:%lu - ppa: %llx: ch:%d,lun:%d,blk:%d,pg:%d,pl:%d,sec:%d\n",
				i,
				ppa.ppa,
				ppa.g.ch,
				ppa.g.lun,
...
static ssize_t pblk_sysfs_cleanup(struct pblk *pblk, const char *buf,
				  ssize_t len)
{
	struct pblk_lun *rlun;
	struct nvm_lun *lun;
...
static struct attribute sys_rate= {
	.name = "rate",
		       ...
		       int lun_begin, int lun_end)
{
	...
	atomic_set(&pblk->inflight_user, 0);
	init_waitqueue_head(&pblk->wait);
	init_waitqueue_head(&pblk->wait2);
